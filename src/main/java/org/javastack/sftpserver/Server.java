...
public class Server implements PasswordAuthenticator, PublickeyAuthenticator {
	...
	private void setupMaxPacketLength() {
		...
		if (maxPacketLength > 1) {
			PropertyResolverUtils.updateProperty(sshd, SftpSubsystem.MAX_READDATA_PACKET_LENGTH_PROP, maxPacketLength);
		}
	}
	...
	static class CustomSftpSubsystemFactory extends SftpSubsystemFactory {
		@Override
		public Command create() {
			final SftpSubsystem subsystem = new SftpSubsystem(getExecutorService(), getUnsupportedAttributePolicy(),
